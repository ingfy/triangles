var game = {
    ...
    createWorld: function() {
        ...
        var distance = {
            start: 5, 
            stop: Math.sqrt(Math.pow(s.width/2, 2) + Math.pow(s.height/2, 2)), 
            steps: 40
        };
            ...
            _(centers).each(function(center) {
                ...
                _(_.range(0, numTriangles)).each(function(i) {
                    s.objects.push(newTriangle(
                        ...
                        1*Math.pow(Math.log(r), 2),
                        Color.gradient(
                            Color.gradient(
                                'rgba(200,200,0,0.25)',
                                'rgba(255,0,0,0.25)', 
                                1 - Math.pow(Math.E, -50*Math.pow(r/(distance.stop - distance.start), 2))
                            ),
                            Color.gradient(
                                'rgba(100,200,0,0.25)', 
                                'rgba(200,200,0,0.25)', 
                                Math.sin(i/numTriangles * Math.PI * 8)), 
                            r/(distance.stop - distance.start)),
                        ((1-(i%2))*Math.PI/2 + (i%2) * (-Math.PI/2))
                    ));
                });        
            });
        }); 
